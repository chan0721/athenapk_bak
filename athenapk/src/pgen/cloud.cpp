...
void InitUserMeshData(Mesh *mesh, ParameterInput *pin) {
  ...
  // Add passive scalar field for easier access in output (similar to turbulence setup)
  const auto &hydro_pkg = mesh->packages.Get("Hydro");
  auto m = parthenon::Metadata({parthenon::Metadata::Cell, parthenon::Metadata::OneCopy}, std::vector<int>({1}));
  if (pin->GetOrAddBoolean("problem/cloud", "calc_passive_scalar", false)) {
    hydro_pkg->AddField("passive_scalar", m);
  }
}
...
void UserWorkBeforeOutput(MeshBlock *pmb, ParameterInput *pin,
  ...
  if (pin->GetOrAddBoolean("problem/cloud", "calc_passive_scalar", false)) {
    ...
    pmb->par_for(
        "Cloud::UserWorkBeforeOutput calc passive_scalar", kb.s, kb.e, jb.s, jb.e, ib.s,
