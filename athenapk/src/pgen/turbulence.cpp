...
void ProblemInitPackageData(ParameterInput *pin, parthenon::StateDescriptor *pkg) {
  ...
  // Add passive scalar field for easier access in output
  if (pin->GetOrAddBoolean("problem/turbulence", "calc_passive_scalar", false)) {
    pkg->AddField("passive_scalar", m);
  }
  ...
  auto inject_once_at_time =
      pin->GetOrAddReal("blob_injection", "inject_once_at_time", -1.0);
  auto inject_once_at_cycle =
      pin->GetOrAddInteger("blob_injection", "inject_once_at_cycle", -1);
  auto inject_once_on_restart =
      pin->GetOrAddBoolean("blob_injection", "inject_once_on_restart", false);
  ...
  auto inject_n_blobs = pin->GetOrAddInteger("blob_injection", "inject_n_blobs", -1);
  pkg->AddParam<>("turbulence/inject_n_blobs", inject_n_blobs);

  for (int i = 0; i < inject_n_blobs; i++) {
    auto inject_blob_radius =
        pin->GetReal("blob_injection", "inject_blob_radius_" + std::to_string(i));
    pkg->AddParam<>("turbulence/inject_blob_radius_" + std::to_string(i),
                    inject_blob_radius);

    auto inject_blob_loc = pin->GetVector<Real>("blob_injection",
    ...
    auto inject_blob_chi =
        pin->GetReal("blob_injection", "inject_blob_chi_" + std::to_string(i));
    ...
    pmb->par_for(
        ...
        ib.s, ib.e, KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
          ...
          if (r < radius) {
            ...
            // Store original density before modification
            const auto rho_orig = cons(IDN, k, j, i);
            ...
            // Update passive scalars - mark blob material with enhanced density
            for (auto n = nhydro; n < nhydro + nscalars; n++) {
              // Set scalar to the enhanced density value to track blob material
              cons(nhydro, k, j, i) = 1.0 ;
            }
          }
        });
  }
...
void UserWorkBeforeOutput(MeshBlock *pmb, ParameterInput *pin,
  ...
  if (pin->GetOrAddBoolean("problem/turbulence", "calc_passive_scalar", false)) {
    ...
    auto &passive_scalar = data->Get("passive_scalar").data;
    ...
    IndexRange kb = pmb->cellbounds.GetBoundsK(IndexDomain::entire);
    pmb->par_for(
        "Turbulence::UserWorkBeforeOutput calc passive_scalar", kb.s, kb.e, jb.s, jb.e, ib.s,
        ib.e, KOKKOS_LAMBDA(const int k, const int j, const int i) {
          // Extract the first passive scalar (if available)
          if (nscalars > 0) {
            passive_scalar(k, j, i) = cons(nhydro, k, j, i);
          } else {
            passive_scalar(k, j, i) = 0.0;
          }
        });
  }
